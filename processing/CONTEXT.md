...
- `ProcessContract` wraps `Contract` metadata plus staging fields (`buyer_text`, `contract_type_text`, `sales_class_text`, `plan_gross`, `planned_split`, `final_contract`, `pr_number`). Status choices (`draft` → `completed`) track workflow state. `pr_number` is carried from `QueueContract`, editable in the processing form via `cont_pr_number`, and copied to `Contract` on finalization. Key methods: `calculate_contract_value`, `calculate_plan_gross`, and `update_calculated_values`, which power the `update_contract_values` API and ensure `contract_value`/`plan_gross` reflect CLIN totals. `calculate_plan_gross` returns `item_total − quote_total` aggregated across CLINs; `packhouse_quote_amount` is **not** part of this formula. Packaging costs are handled on the Finance side, so the processing pipeline captures the packhouse quote for finalization data carry-over only and does not deduct it from `plan_gross`. **`total_split_value` / `total_split_paid`** on `ProcessContract` are computed aggregates over all `ProcessClinSplit` rows for its CLINs (not stored columns on `ProcessContract`). **Packhouse (optional):** `packhouse` (FK `Supplier`, PROTECT, nullable, `related_name='+'`), `packhouse_quote_amount` (Decimal, nullable), `packhouse_notes` (Text, nullable). Analysts assign via the packhouse picker modal on the process form. The quote amount is captured during processing and copied to `ContractPackaging` on finalization, but it does **not** affect `plan_gross`. On finalization, if `packhouse_id` is set, a `ContractPackaging` row is created on the new `Contract` with `quote_amount` and `notes` copied; if no packhouse, no `ContractPackaging` row is created.
