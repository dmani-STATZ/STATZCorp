...
3. **Start processing:** `initiate_processing` marks `QueueContract` as locked, `start_processing` clones queue data into `ProcessContract`/`ProcessClin` (PO/Tab fields left `None` until finalization), and flags the queue item as in-progress. Before creating `ProcessContract`, if `QueueContract.idiq_number` is set, `start_processing` looks up `IdiqContract` with `contract_number__iexact` that hint and sets `ProcessContract.idiq_contract` when a row exists (delivery orders from PDFs auto-link without opening the IDIQ modal). `start_new_contract` can create a blank process contract + a default CLIN without queue data. `ProcessClin.item_type` defaults to `'P'` (Production) when creating CLINs via `start_processing` or `add_processing_clin` if source data leaves the field blank.
