...
As of the Bootstrap-in-`<head>` fix in `base_template.html`, the timing race no longer applies to `window.bootstrap` specifically — Bootstrap is available before any `{% block content %}` inline script runs. The `DOMContentLoaded` guard above is still recommended for DOM queries (element existence), not for Bootstrap availability.
...
`finalize_and_email_contract` writes to `contracts.Contract`, `contracts.Clin`, `contracts.ClinSplit` (from each `ProcessClinSplit` on the matching `ProcessClin`), `contracts.PaymentHistory`, `contracts.ContractLevelCharge`, and may create **`contracts.ContractPackaging`** when `ProcessContract.packhouse` is set. Any schema change in those models must be reflected in the finalization mapping in `processing_views.py`.
...
## 17. Release Notes (Changelog) Rules
