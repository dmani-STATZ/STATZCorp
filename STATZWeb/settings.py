...
if IS_PRODUCTION:
    _default_media_root = (
        Path("D:/home/media") if os.name == "nt" else Path("/home/site/media")
    )
...
REQUIRE_LOGIN = (
    os.getenv("REQUIRE_LOGIN", "False" if not IS_PRODUCTION else "True").lower()
    == "true"
)
...
if IS_PRODUCTION:
    ...
    if missing_azure_vars:
        raise ImproperlyConfigured(
            f"Missing required Azure AD environment variables: {', '.join(missing_azure_vars)}"
        )
...
AZURE_AD_CONFIG = {
    ...
    "redirect_uri": os.environ.get(
        ...
        (
            "https://statzweb.azurewebsites.us/microsoft/auth-callback/"
            if IS_PRODUCTION
            else "http://localhost:8000/microsoft/auth-callback/"
        ),
    ),
    "authority": os.environ.get(
        "MICROSOFT_AUTH_AUTHORITY", "https://login.microsoftonline.us"
    ),
    "graph_endpoint": os.environ.get(
        "MICROSOFT_AUTH_GRAPH_ENDPOINT", "https://graph.microsoft.us"
    ),
