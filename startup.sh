...
(
  ...
  for attempt in 1 2 3; do
    ...
    echo "[startup:bg] install-deps attempt $attempt failed, retrying in 5s..."
    sleep 5
  done
  if [ "$INSTALL_DEPS_OK" != "1" ]; then
    echo "[startup:bg] WARNING: install-deps failed after 3 attempts — Chromium launch may fail"
  fi
