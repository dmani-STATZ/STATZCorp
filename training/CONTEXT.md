...
3. **User requirements**: `user_training_requirements` builds `required_courses_data` combining latest `Tracker`, deadline logic, latest `TrainingDoc`, and `CourseReviewClick`; template highlights expired/completed courses, exposes a review link, and per-course certify controls. Recertification is available at any time (not gated on expiration). **Three-way split:** non-cert courses (`Course.upload=False`) use **Click to Certify** → `mark_complete` (creates today's `Tracker` row). Cert-required courses use **Upload to Certify** → `upload_document` (creates today's `Tracker` row and attaches the file). **Replace Document** → `replace_document` swaps the certificate on the latest existing `Tracker` row only — never creates a row or changes `completed_date`. `mark_complete` server-side rejects cert-required courses.
4. **Review/document flows**: `review_course_link` enforces that the request user is tied to the matrix’s account before streaming the latest file or redirecting to an external `link`; `upload_document` certifies/recertifies; `replace_document` corrects a file in place; `view_document` retrieves stored binary data.
...
- AW templates (`arctic_wolf_course_list.html`, `arctic_wolf_completion.html`, `arctic_wolf_completion_status.html`, `user_arctic_wolf_courses.html`, `arctic_wolf_audit.html`, `arctic_wolf_email.html`, `arctic_wolf_email_body.html`, `_arctic_wolf_sign_button.html`): progress bars, copy-to-clipboard controls, email previews, and completion buttons tailored to unique AW links.
- `templates/training/_arctic_wolf_sign_button.html`: shared Sign CTA partial (table-cell button + plain-URL fallback) used by both the preview page and the `.eml` body. Do not duplicate this markup.
- `arctic_wolf_email_body.html` is the single canonical email body (complete HTML document). `arctic_wolf_email.html` is preview chrome only — it includes the body via `{% include %}` so Copy HTML and Download EML cannot diverge.
- Optional setting `TRAINING_EMAIL_BASE_URL`: when set (e.g. `https://app.example.com`), `_absolute_training_url` in `views.py` builds outbound AW email links from that base instead of the request host. When unset, falls back to `request.build_absolute_uri`. Prevents distributing `localhost` / internal-host links when an admin generates email from a non-production host. Not declared in `settings.py` by default — use `getattr` with `None`.
...
- Document flows (`review_course_link`, `mark_complete`, `upload_document`, `replace_document`, `view_document`) ensure only associated users can mark courses, upload attachments, and retrieve stored binary data. Non-cert: `mark_complete`. Cert-required recert: `upload_document` (new row). Cert-required file fix: `replace_document` (latest row only, no date change). Prior `Tracker` rows are never mutated except explicit file swap on the current latest row.
...
- `/training/mark-complete/<course_id>/` -> `mark_complete`: non-cert courses only; rejects `Course.upload=True`.
- `/training/upload-document/<matrix_id>/` -> `upload_document`: cert-required certify/recertify (new `Tracker` row).
- `/training/replace-document/<matrix_id>/` -> `replace_document`: cert-required file swap on latest row (no new row, no date change).
- `/training/view-document/<tracker_id>/` -> document download handler.
...
- Any change to document storage must keep `TrainingDocForm`, `TrainingDocInline`, `upload_document`, `replace_document`, `admin_cmmc_upload`, and the review link logic aligned. Recertification always inserts a new `Tracker` row via `get_or_create` (`mark_complete` or `upload_document`); `replace_document` must never create a row or alter `completed_date`. Keep `mark_complete` rejection, `upload_document`/`replace_document` split, and `user_requirements.html` controls in sync if `Course.upload` semantics change.
