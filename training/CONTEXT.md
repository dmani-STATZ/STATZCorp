...
3. **User requirements**: `user_training_requirements` builds `required_courses_data` combining latest `Tracker`, deadline logic, latest `TrainingDoc`, and `CourseReviewClick`; template highlights expired/completed courses, exposes a review link, file upload controls (`upload_document`), and a `mark_complete` form. Recertification is available at any time (not gated on expiration). **Split by cert requirement:** courses with `Course.upload=True` recertify exclusively through certificate upload (`upload_document` creates a new `Tracker` row for today and attaches the file — upload is the recert action; there is no separate Recertify button). Courses with `Course.upload=False` use **Recertify Early** / **Recertify Now** via `mark_complete`, which is server-side rejected for cert-required courses. Each recertification inserts a new `Tracker` row rather than mutating an existing one; same-day repeat actions reuse today's row via `get_or_create`.
4. **Review/document flows**: `review_course_link` enforces that the request user is tied to the matrix’s account before streaming the latest file or redirecting to an external `link`; `upload_document` creates or reuses today's `Tracker` row then stores the certificate; `view_document` retrieves stored binary data.
...
- Document flows (`review_course_link`, `mark_complete`, `upload_document`, `view_document`) ensure only associated users can mark courses, upload attachments, and retrieve stored binary data. For cert-required courses (`Course.upload=True`), `upload_document` is the sole recert path (creates today's `Tracker` via `get_or_create`, then attaches the file); `mark_complete` rejects these courses server-side. Non-cert courses use `mark_complete` only. Prior `Tracker` rows and their certificates are never mutated.
...
- `/training/mark-complete/<course_id>/` -> `mark_complete`: creates a new `Tracker` row for non-cert courses only; rejects cert-required courses (`Course.upload=True`).
- `/training/upload-document/<matrix_id>/` & `/training/view-document/<tracker_id>/` -> cert upload (creates today's `Tracker` + attaches file) and document download handlers.
...
- Any change to document storage must keep `TrainingDocForm`, `TrainingDocInline`, `upload_document`, `admin_cmmc_upload`, and the review link logic aligned so uploads remain discoverable. Do not change recertification to in-place `completed_date` mutation — always insert a new `Tracker` row via `get_or_create`. If `Course.upload` semantics change, keep `mark_complete` rejection, `upload_document` create-and-attach behavior, and `user_requirements.html` gating in sync.
