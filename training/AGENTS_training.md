...
- **Recertification creates new `Tracker` rows** — `mark_complete` (non-cert) and `upload_document` (cert-required) insert a new row via `get_or_create`. **`upload_document` and `replace_document` are not interchangeable** — `upload_document` always recertifies; `replace_document` only swaps the file on the latest row and must never create a row or change `completed_date`. Mixing them up would silently recertify a typo fix or fail to recertify an intended upload.
