...
class Matrix(models.Model):
    ...
    frequency = models.CharField(
        max_length=100, blank=True, null=True, choices=FREQUENCY_CHOICES
    )
...
class UserAccount(models.Model):
    ...
    class Meta:
        unique_together = (
            "user",
            "account",
        )  # Ensure a user can't be linked to the same account twice
...
class Tracker(models.Model):
    ...
    def __str__(self):
        return (
            f"{self.user.username} - {self.matrix} (Completed: {self.completed_date})"
        )
...
class ArcticWolfCompletion(models.Model):
    ...
    class Meta:
        unique_together = (
            "user",
            "course",
        )  # Ensure a user can't have multiple completion records for the same course
...
class CourseReviewClick(models.Model):
    ...
    reviewed_doc = models.ForeignKey(
        "TrainingDoc",
...
class TrainingDoc(models.Model):
    ...
    def __str__(self):
        return f"{self.course.name} - {self.file_name}"
