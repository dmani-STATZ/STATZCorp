...
- **ClinSplit.percentage:** Nullable decimal. Do NOT treat it as a derived field or compute it from `split_value`. It is user-entered. **`split_value`** is auto-computed at contract creation (when splits are present) via `recalc_split_values()` in `contracts/services/contract_create.py`, which distributes packaging- and charge-adjusted adj gross by company percentage and CLIN `item_value` weight. The Finance Audit **Recalc Split** button calls the same function via the `recalc_splits` view — do not duplicate the distribution logic elsewhere. The `recalc_splits` view resolves NULL percentages at recalc time — it does NOT require all percentages to be set in advance. See CONTEXT.md for the three-case resolution logic.
