...
- `contracts/services/sharepoint_paths.py` — strict validation (`is_modern_sharepoint_path`), structured resolution (`resolve_contract_folder_path` returns `{path, source, legacy_detected}`). IDIQ-only rows use `build_idiq_pattern_path` / `resolve_idiq_folder_path` (no `Company` FK). Use `join_path()` for IDIQ pattern segments only. **`build_explorer_uri()`** maps drive-relative SharePoint paths to `statzfile:///` URIs for local OneDrive Explorer opens (pure, no I/O).
...
- `contracts/views/contract_views.py` — **`ContractManagementView`** passes **`explorer_uri`** (`build_explorer_uri` on `contract.get_sharepoint_relative_path()`) into `contract_management.html` for the Documents split button; empty string when the path is legacy/unmappable (template disables Explorer).
