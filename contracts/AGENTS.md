...
- **ContractLevelCharge PO seeding and UI (2026-06):** Rows seed into `POLineItem` only at PO creation time (same RUNS ONCE guard as CLIN seeding in `_seed_po_lines_from_clins`). Reopening the PO page does not re-seed. The charge detail panel partial loads via AJAX into `#clin-details-content` on Contract Management. Selecting a charge row deselects CLIN rows and vice versa. All six charge fields are transaction-tracked; keep `transactions/signals.py` `TRACKED` and `store_old_state` in sync when adding fields.
...
- Add Charge modal on Contract Management pre-populates the supplier from `main_supplier` context. This is UI-only convenience — supplier FK is nullable at the DB level.
