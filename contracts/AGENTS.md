...
- **`add_partial_modal.js`:** Must work on any page that includes `add_partial_modal.html`. Never add page-specific logic to this file; page-specific behavior after save (if ever needed) should use a page-level listener on the `#addPartialModal` `hidden.bs.modal` event.
