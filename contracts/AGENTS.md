...
- **Finance Audit accordion breakdown (2026-06):** `clin_splits_by_company` rows now carry `raw_split_value` (CLIN adj_gross × %, computed in `get_context_data` with `finance_lines` prefetched). Template shows raw values in CLIN child rows and a Packaging deduction row per company when `packaging_share_per_company[company]` is non-zero. Company header and Total row remain packaging-adjusted (stored `split_value`). Do not swap these — the breakdown must add up: Σ `raw_split_value` − `packaging_share` = company header total.
