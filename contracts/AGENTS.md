...
- `contracts/views/payment_history_views.py` — the `delete_payment_history_entry` view recalculates and writes back totals to the parent model on delete using the same field-write logic as the POST handler. If you add new payment types or entity types to PaymentHistory in the future, update both the POST handler AND this delete handler.
...
- **`Clin.adjusted_gross` formula (2026-05-18):** Income side uses `item_value` only — `wawf_payment` no longer participates in adj_gross (it remains on the CLIN for informational/reporting). Cost side uses `COALESCE(paid_amount, quote_value)` — `paid_amount` is the "official" number once populated and non-zero. Finance costs (all `ContractFinanceLine.amount_billed` for the CLIN, both CLIN-level and partial-scoped) are subtracted last. This may be revisited when finance users want realized cash-flow reporting on the income side. Do **not** add a stored field for this — it is and must remain a computed `@property`. The async refresh endpoints (`finance_audit_clin_api`, `finance_audit_summary_api`) and the page-load template all read `clin.adjusted_gross` directly, so the property fix propagates everywhere without endpoint-level changes.
- **`Contract.adjusted_gross` (2026-05-15):** Formula is `SUM(Clin.adjusted_gross) - packaging_variance` (context/API still expose this as `packaging_deduction` for template/JS compatibility). `packaging_variance = amount_paid - quote_amount` when `amount_paid` is set and non-zero; otherwise `0`. Positive variance means overage (paid above quote) and **reduces** contract adj gross; negative variance means savings and **increases** it. The quote amount is already absorbed into `plan_gross` upstream in Processing — do not deduct `quote_amount` alone on the Finance Audit side. `plan_gross` does NOT drive contract adj gross. Finance costs are already inside `Clin.adjusted_gross` — do not subtract `finance_costs_total` at the contract level or you will double-count. `FinanceAuditView` and `finance_audit_summary_api` must stay in sync with the property. The CRITICAL sync requirement note in AGENTS.md still applies: if you change the aggregation logic in `get_context_data()`, update `finance_audit_summary_api()` with the same logic simultaneously. Do **not** add a stored `adjusted_gross` field on `Contract` — it is and must remain a computed `@property`.
...
- **Payment history for partials — rollup behavior:** For `payment_history_api` POST/DELETE with `entity_type='clinshipment'` and payment types `partial_wawf_payment` or `partial_paid_amount`, the view writes to the `ClinShipment` field AND creates/deletes a corresponding `PaymentHistory` row on the parent `Clin` (types `wawf_payment` / `paid_amount`). The CLIN row's `payment_info` is set to `'From Shipment #<id>'` and `reference_number` to `'shipment-ph-<shipment_payment_history_id>'` for delete tracking. The CLIN stored field is recalculated from the SUM of all its PaymentHistory entries after each operation. For `partial_item_value` and `partial_quote_value`, no rollup occurs — those remain shipment-only.
...
- **`add_partial_modal.js`:** Must work on any page that includes `add_partial_modal.html`. The modal supports a CLIN selector mode when `window.financeClinList` is defined (Finance Audit context). In this mode the CLIN selector drives `#addPartialClinId`, UOM population, and auto-calc. When opened from a single-CLIN context (CLIN detail), selector is hidden and CLIN id comes from `data-clin-id` on the trigger button as before. The `name` field is always present in the modal; value is passed in the POST body to `add_partial_shipment`. Page-specific behavior after save (if ever needed) should use a page-level listener on the `#addPartialModal` `hidden.bs.modal` event.
- **`log_payment_modal.js`** depends on `window.financeClinList` and `window.financeShipmentsByClin` both being defined on the Finance Audit page. It uses the existing `add_partial_shipment` POST endpoint and `payment_history_api` POST endpoint — no new backend endpoints. The `+ add` finance pill (empty state) has been removed from CLIN rows; the Finance column is empty when no finance lines exist.
- **`add_finance_line_modal.js`** depends on `window.financeClinList` and `window.financeShipmentsByClin` being defined on the Finance Audit page (same globals used by `log_payment_modal.js`). It chains two existing endpoints: `add_finance_line` (POST `/contracts/api/finance-lines/add/`) or `add_partial_finance_line` when shipment-scoped, then `log_finance_line_payment` (POST `/contracts/api/finance-lines/<id>/log-payment/`). If Amount Paid is 0, the payment POST is skipped. Amount Paid mirrors Amount Billed by default but tracks manual edits via `data-manually-edited` attribute to avoid overwriting user input. The `add_finance_line` endpoint must return `finance_line_id` in its success response — verify this before running.
...
**DFAS payment import (Phase 1 + 2, 2026-05):** Pipeline is `dfas_parser.py` (parse) → `dfas_matcher.py` (match) → `dfas_import.py` (persist + `finalize_import_batch`). Extend matching in `dfas_matcher` only; do not duplicate **IDIQ → delivery order** suffix stripping — use `strip_delivery_order_suffix`. `PaymentHistory.payment_amount` has **no** minimum validator (negative DFAS reversals allowed app-wide); do not reintroduce `MinValueValidator` without an explicit product decision. `DfasImportBatch` / `DfasImportRow` are audit tables: do not `bulk_update`, do not delete in normal flows, do not hand-edit via ORM in application code. **UX:** `dfas_import_views.py`, templates under `contracts/dfas_import_*.html` + `dfas_contract_matcher_modal.html`, JS `dfas_import_review.js`. **New modals:** use Bootstrap 5 `modal` / `modal-dialog` components — not the legacy Tailwind-utility overlay pattern in `idiq_modal.html` and `nsn_modal.html` (those are on the modernization backlog). **Contract lookup:** prefer the existing JSON endpoint **`contracts:contract_search`** before adding a new search route. **Per-row DFAS resolutions:** add new `action` branches in `dfas_import_resolve_row_view` only; avoid separate URL patterns per action.
