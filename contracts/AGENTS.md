...
- **`_sync_clin_ship_fields`:** Always call `_sync_clin_ship_fields` after creating a `ClinShipment` (including from `add_partial_shipment` and `create_shipment`) so CLIN `ship_qty` and `ship_date` stay accurate as rollups of child shipments.
...
- `contracts/context_processors.py` — `reminders_processor` provides the sidebar with `reminders`, `overdue_count`, `due_count`, `pending_count`, `total_reminders_count`, and `reminder_sidebar_upcoming_days`. Each sidebar reminder is annotated with `is_overdue`, `is_upcoming`, `title`, `description`, and `completed` (do not rename these in the processor without updating the template). **Footer pill only:** `footer_overdue_count` (non-completed, `reminder_date__date < today`, no 7-day cap) and `footer_due_today_count` (non-completed, date exactly today). Footer badge sum = `footer_overdue_count + footer_due_today_count`; pill color red if `footer_overdue_count > 0`, else green. Do not repurpose `due_count` / `overdue_count` / `total_reminders_count` for the footer pill — sidebar and popup depend on their existing definitions.
- `contracts/templates/contracts/contract_base.html` — sticky-footer **Reminders** pill (not the sidebar panel below it) uses `footer_overdue_count` / `footer_due_today_count` for badge visibility, count, and red/green styling as above.
...
- Note **delete** still requires `request.user == note.created_by or request.user.is_staff` in views (`delete_note`). **Edit** in `note_update` remains creator **or** staff at the view layer. **UI:** In **`partials/notes_popup_tab_panel.html`** and **`partials/notes_list.html`** (contract management), the **Edit** button is shown only to the note creator (`request.user == note.created_by`) — no staff exception in those templates (staff may still save via direct POST if they bypass the UI). Reminder creation/edit/delete on a Note in `note_update` / `add_note` is unchanged (creator or staff where applicable). The meta-row **reminder pill** (when `current_user_has_reminder`) and the full note body open **`#noteFullViewModal`**; Mark Complete / Incomplete POSTs to `contracts:toggle_reminder`.
