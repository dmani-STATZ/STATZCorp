...
- Packaging destination now creates `ContractLevelCharge` (`label='Packaging'`), not `ContractPackaging`. The "existing packaging" and "multiple-packaging guard" rules have been removed from `_validate_conversions()`. Income-side guard remains.
- Tests for the removed guards (duplicate packaging, multiple-packaging-in-batch) have been deleted. `test_clin_to_packaging_field_mapping` has been updated to assert `ContractLevelCharge` instead of `ContractPackaging`.
