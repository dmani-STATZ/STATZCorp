...
from ..models import Clin, ClinAcknowledgment, ClinSplit, Supplier, ExportTiming, Buyer, ContractType, ContractLogExportJob
...
def export_contract_log_xlsx(request):
    ...
    from ..services.contract_log_export import generate_contract_log_xlsx

    filters_applied = {k: v for k, v in request.GET.items() if k not in ('page', 'per_page') and v}
    file_bytes, row_count = generate_contract_log_xlsx(request.active_company, request.GET)
...
@conditional_login_required
@require_POST
def request_contract_log_export(request):
    """Queue an async Contract Log export job, picked up by the process_contract_log_exports WebJob."""
    filters_applied = {k: v for k, v in request.GET.items() if k not in ('page', 'per_page') and v}
    job = ContractLogExportJob.objects.create(
...
@conditional_login_required
def contract_log_export_status(request, job_id):
    """Poll the status of a queued/running Contract Log export job."""
    try:
        job = ContractLogExportJob.objects.get(pk=job_id, company=request.active_company)
    except ContractLogExportJob.DoesNotExist:
        raise Http404
...
@conditional_login_required
def download_contract_log_export(request, job_id):
    """Download a completed Contract Log export file."""
    try:
        job = ContractLogExportJob.objects.get(pk=job_id, company=request.active_company)
    except ContractLogExportJob.DoesNotExist:
        raise Http404

    if job.status != ContractLogExportJob.STATUS_DONE or not job.file_path or not os.path.exists(job.file_path):
        raise Http404

    return FileResponse(
        open(job.file_path, 'rb'),
        as_attachment=True,
        filename='contract_log.xlsx',
        content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
    )
