...
- **Omnibox CAGE search:** supplier lookup goes through `_suppliers_matching_cage()` (`__iexact` + bounded strip fallback). When changing `portal_search` classifier order or CAGE tokenization, run `products.tests.test_search` — padded `cage_code` values and hyphenated input are regression-tested. `_search_cage()` now falls back to `_search_text()` on a total miss (no supplier and no SAM hit), so any future change to CAGE tokenization or the CAGE bucket's early-return behavior must preserve that fallback or non-standard `nsn_code` values will silently become unsearchable again.
