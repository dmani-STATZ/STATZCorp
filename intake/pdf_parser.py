"""Intake-owned DD Form 1155 award PDF parsing.

This module intentionally has no dependency on the processing app. Callers use
``parse_award_pdf()`` and map the returned dataclasses in ``intake.ingest``.
"""
...
def detect_contract_type(contract_number: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        if len(parts) >= 3 and len(parts[2]) == 1:
            return _CONTRACT_TYPE_MAP.get(parts[2].upper())
    except Exception:
        logger.exception(
...
class ClinParseResult:
    ...
    clin_parse_note: Optional[str]
    min_order_qty_text: Optional[str]
...
class AwardParseResult:
    ...
    solicitation_type: Optional[str]
    pr_number: Optional[str]
    pdf_parse_status: Optional[str]
    pdf_parse_notes: Optional[str]
    ado_days: Optional[int]
    clins: list
    idiq_max_value: Optional[Decimal]
    idiq_min_guarantee: Optional[Decimal]
    idiq_term_months: Optional[int]
    idiq_option_months: Optional[int]
...
def _normalize_nsn(nsn: Optional[str]) -> Optional[str]:
    """Normalize an NSN string to XXXX-XX-XXX-XXXX; pass S-codes through."""
    if not nsn:
        return None
...
def _read_pdf_bytes(pdf_file: PdfInput) -> bytes:
    if isinstance(pdf_file, (str, os.PathLike)):
        with open(os.fspath(pdf_file), "rb") as fh:
            return fh.read()
    pos = getattr(pdf_file, "tell", lambda: None)()
    pdf_file.seek(0)
    data = pdf_file.read()
...
def _extract_with_pypdf(pdf_file: PdfInput) -> tuple[str, str]:
    from pypdf import PdfReader

    reader = PdfReader(io.BytesIO(_read_pdf_bytes(pdf_file)))
    parts: List[str] = []
    page_one = ""
...
def _extract_pdf_texts(pdf_file: PdfInput) -> tuple[str, str]:
    if pdfplumber is not None:
        try:
            with _open_pdfplumber(pdf_file) as pdf:
                return _extract_full_text(pdf), _safe_page_one_text(pdf)
        except Exception:
...
_RE_INSPECTION_SUPPLIES_LABEL = re.compile(
    r"PLACE\s+of\s+INSPECTION\s+for\s+SUPPLIES\s*:",
    re.IGNORECASE,
)
_RE_INSPECTION_PACKAGING_LABEL = re.compile(
    r"PLACE\s+of\s+INSPECTION\s+for\s+PACKAGING\s*:",
    re.IGNORECASE,
)


def _extract_inspection_party(
    text: str,
    label_re: re.Pattern,
) -> tuple[Optional[str], Optional[str]]:
    """Return first two non-blank lines after a PLACE OF INSPECTION label."""
    if not text:
        return None, None
    ...
    cage = lines[0].upper() if len(lines) >= 1 else None
    name = lines[1] if len(lines) >= 2 else None
    return cage, name


def _extract_supplies_party(text: str) -> tuple[Optional[str], Optional[str]]:
    return _extract_inspection_party(text, _RE_INSPECTION_SUPPLIES_LABEL)


def _extract_packaging_party(text: str) -> tuple[Optional[str], Optional[str]]:
    return _extract_inspection_party(text, _RE_INSPECTION_PACKAGING_LABEL)


def _clin_supplier_values(
    chunk: str,
    contract_supplier_cage: Optional[str],
    contract_supplier_name: Optional[str],
) -> tuple[Optional[str], Optional[str]]:
    cage, name = _extract_supplies_party(chunk)
    return cage or contract_supplier_cage, name or contract_supplier_name


def _clin_chunk_for_item(section: str, item_number: Optional[str]) -> str:
    if not item_number:
        return ""
    item = str(item_number).strip().zfill(4)
    patterns = (
        rf"(?:^|\n)\s*CLIN\s+{re.escape(item)}\s+PR\s*:",
        rf"(?:^|\n)\s*(?:CLIN\.\s*)?{re.escape(item)}\s+\d{{6,15}}\s+\d{{1,4}}\s+\w+\s+[\d.]+",
        rf"(?:^|\n)\s*{re.escape(item)}\s+",
    )
    for pattern in patterns:
        ...
        if m:
            start = m.start()
            _, end = _clin_chunk_bounds(section, start)
            return section[start:end]
    return ""


def _apply_clin_inspection_drilldown(
    ...
    for clin in clins:
        chunk = _clin_chunk_for_item(section, clin.item_number)
        cage, name = _clin_supplier_values(
            chunk,
            contract_supplier_cage,
            contract_supplier_name,
        )
        clin.cage = cage
        clin.supplier_name = name
...
def _clin_chunk_bounds(text: str, start: int) -> tuple[int, int]:
    """End before the next CLIN row/header or SECTION."""
    rest = text[start:]
    mnext = re.search(
        r"(?:^|\n)\s*(?:"
        r"CLIN\s+\d{4}\s+PR\s*:|"
        r"(?:CLIN\.\s*)?\d{4}\s+\d{6,15}\s+\d{1,4}\s+\w+\s+[\d.]+|"
        r"\d{4}\s+[A-Z0-9\-]+(?:\s*-\s*[A-Z0-9]+)?\s+\d+(?:\.\d+)?\s+\w+\s+\$?\s*[\d,]+"
        r")",
        rest[50:],
        re.IGNORECASE | re.MULTILINE,
    )
...
def _parse_clins_from_text(
    ...
    contract_packhouse_cage: Optional[str] = None,
    ...
    if api_clins:
        ...
        _apply_clin_inspection_drilldown(
            clins_from_api,
            section,
            contract_supplier_cage,
            contract_supplier_name,
        )
        return clins_from_api, contract_packhouse_cage
    clins: List[ClinParseResult] = []
    ...
    for i, line in enumerate(lines):
        ...
        supplier_cage, supplier_name = _clin_supplier_values(
            chunk,
            contract_supplier_cage,
            contract_supplier_name,
        )
        _extract_packaging_party(chunk)
    ...
    return clins, contract_packhouse_cage
...
def parse_award_pdf(pdf_file: PdfInput) -> AwardParseResult:
    ...
    try:
        ...
        contract_supplier_cage, contract_supplier_name = _extract_supplies_party(text)
        packaging_cage, contract_packhouse_name = _extract_packaging_party(text)

        clins: List[ClinParseResult] = []
        try:
            clins, packaging_cage = _parse_clins_from_text(
                text,
                contract_supplier_cage=contract_supplier_cage,
                contract_supplier_name=contract_supplier_name,
                contract_packhouse_cage=packaging_cage,
            )
            for c in clins:
