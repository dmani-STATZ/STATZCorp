"""Atomic PO number minting from the intake_sequencenumber table."""


def mint_intake_po_number() -> int:
    """Atomically increment and return the next PO number from the
    intake_sequencenumber table (single row, id=1).
    ...
    with connection.cursor() as cursor:
        if connection.vendor == 'microsoft':
            ...
            if cursor.rowcount == 0:
                cursor.execute(
                    "INSERT INTO intake_sequencenumber (id, po_number, tab_number) "
                    "VALUES (1, 10001, 10000)"
                )
            cursor.execute(
                "SELECT po_number FROM intake_sequencenumber WHERE id = 1"
            )
            row = cursor.fetchone()
    if row is None:
        raise RuntimeError(
            "intake_sequencenumber table returned no row — "
