...
- `share_report.html` — Dedicated share page. Report summary card at top. Share form with native `<select>` for recipient and `can_branch` checkbox. Existing shares list with Revoke button. Each existing share renders a POST form targeting {% url 'reports:revoke_share' share_pk=share.id %}. The view is fully implemented: ownership check, share.delete(), success message, redirect back to reports:share_report.
