...
`Contact` holds the name/title/company/phone/email for a supplier, optionally linked to a `contracts.Address` and back to `Supplier` via `contacts`. The `is_primary` BooleanField (nullable, default `False`) is the source of truth for which contact(s) represent the supplier’s primary contact(s). Multiple contacts per supplier may have `is_primary=True` by design — no uniqueness constraint. Legacy `isPrime` data was backfilled via SQL from `migrate_contact` / `migrate_supplier`. Canonical lookup: `Contact.objects.filter(supplier=supplier, is_primary=True).first()` (or `.values_list('id', flat=True).first()` for IDs). Toggle via POST `contracts:supplier_toggle_contact_primary`.
