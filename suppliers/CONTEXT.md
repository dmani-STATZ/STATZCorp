...
| `migrations/` | `0001_initial` bootstraps tables with `contracts_*` naming; `0002`–`0003` add enrichment/AI model settings; `0004` adds `Supplier.rfq_email`; `0005` added `SupplierContactGroup` (removed in `0011`); `0009`–`0012` add `SupplierContactCategory`, migrate legacy data, and drop `Contact.is_primary`. |
...
- Core history: `0001_initial` (creates the `contracts_*` tables and wires FKs to `contracts.Address`, `SpecialPaymentTerms`, and `User`), `0002` (adds `logo_url`, `website_url`, `primary_email`, `primary_phone`, and `last_enriched_at` to `Supplier`), `0003` (creates `suppliers.OpenRouterModelSetting` with the `needs_update` flag), `0004` (adds `Supplier.rfq_email`), `0005` (added `SupplierContactGroup`, removed in `0011`), `0007` (added `Contact.is_primary`, removed in `0012`), `0009`–`0012` (contact categories schema, data migration, group teardown).
