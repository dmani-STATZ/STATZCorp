...
`Supplier` maps to `contracts_supplier` and centralizes the supplier metadata: name, `cage_code`, `dodaac`, phone/email/web, status flags (`probation`, `conditional`, `archived` plus tracking fields), optional `special_terms`, `prime`, certifications (`ppi`, `iso`), `allows_gsi` choices, references to three `contracts.Address` rows (billing/shipping/physical), a deprecated `Contact` FK (`Supplier.contact`, pending Stage 2 removal), optional `packhouse` self-reference, `logo_url`, `last_enriched_at`, and the `files_url`/`notes` fields that drive detail templates.
...
`Contact` holds the name/title/company/phone/email for a supplier, optionally linked to a `contracts.Address` and back to `Supplier` via `contacts`. The `is_primary` BooleanField (nullable, default `False`) reintroduces the legacy `isPrime` flag from `COMMONCORE.dbo.STATZ_SUPPLIER_CONTACT_TBL` that was not carried over in the original data migration. Multiple contacts per supplier may have `is_primary=True` by design — no uniqueness constraint. Values are populated post-deploy via a manually-run SQL script mapping through `migrate_contact` and `migrate_supplier` (1,136 legacy primary rows; 19 excluded due to supplier ID mismatches). For primary-contact lookups in new code, use `Contact.objects.filter(supplier=supplier, is_primary=True)` rather than `Supplier.contact`.
...
- `templates/suppliers/supplier_detail.html` – two-column desktop layout: a 220px sticky left sidebar (`var(--company-primary)` background) with the current supplier name, a jump search (`#sidebar-supplier-search` → `/suppliers/search/`, min 2 characters, up to 8 results), and anchor nav with count badges (contacts, contracts, certifications, documents). Main content scrolls in `.supplier-detail-main` with eight anchored sections: `#section-overview` (status flag banners when probation/conditional/archived are true; identity block; supplier type/prime/packhouse; contact & web; health gauge), `#section-compliance` (status flags and compliance details sub-groups), `#section-contacts` (contacts, RFQ email picker, contact groups), `#section-addresses`, `#section-contracts`, `#section-certs` (certifications and classifications side-by-side), `#section-files`, `#section-notes`. IntersectionObserver scroll spy highlights the active sidebar link (≥30% visible). Conditional status banners use `var(--bs-danger)` / `var(--bs-warning)` / `var(--bs-secondary)` as appropriate. Prime displays `prime_display` when provided, otherwise `—` for a set integer with no label (template-level; no raw ID). Edit Supplier link removed; inline transaction modal editing retained. Hidden `#enrichment-panel` preserved.
...
- Core history: `0001_initial` (creates the `contracts_*` tables and wires FKs to `contracts.Address`, `SpecialPaymentTerms`, and `User`), `0002` (adds `logo_url`, `website_url`, `primary_email`, `primary_phone`, and `last_enriched_at` to `Supplier`), `0003` (creates `suppliers.OpenRouterModelSetting` with the `needs_update` flag), `0004` (adds `Supplier.rfq_email`), `0005` (creates `SupplierContactGroup` + `contracts_suppliercontactgroup_contacts`), `0007` (adds `Contact.is_primary` on `contracts_contact`).
