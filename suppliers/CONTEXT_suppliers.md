...
- Define supplier-related domain objects (`Supplier`, `Contact`, `SupplierAlias`, `SupplierDocument`, certifications/classifications, and OpenRouter model settings) that reference `contracts.Address`, `Contract`, `Clin`, and `SpecialPaymentTerms`.
...
| File | Role |
| --- | --- |
| `models.py` | Defines `AuditModel`, `Supplier` (with contract-related fields, status flags, GSI choices, address FKs), `SupplierType`, `Contact`, `SupplierAlias` (manual AKA/DBA/legacy names), certifications/classifications/documents, and `OpenRouterModelSetting` with the shared `key`/`needs_update` flag. Many tables map to existing `contracts` schema via `db_table`. |
...
| `admin.py` | Registers `Supplier` (with `SupplierAlias` TabularInline) and `Contact` with tailored `list_display`, search, filters, and grouped `fieldsets` for compliance metadata. |
...
| `portal/` | Server-to-server supplier portal API (`auth`, `views`, `serializers`, `audit`, `notify`, `downloads`). Mounted at `/api/supplier-portal/v1/` in root urls. Includes `GET …/suppliers/{cage}/contracts/` (allowlisted contract/CLIN read), plus `POST send-email/` which calls `mailer.services.graph_mail.send_mail_via_graph` as HTML from `GRAPH_MAIL_SENDER_CONTRACT`. |
| `migrations/` | `0001_initial` bootstraps tables with `contracts_*` naming; `0002`–`0003` add enrichment/AI model settings; `0004` adds `Supplier.rfq_email` (deprecated — dormant fallback after `0013`); `0005` added `SupplierContactGroup` (removed in `0011`); `0009`–`0013` add `SupplierContactCategory`, migrate legacy data, drop `Contact.is_primary`, and backfill Sales contacts from `rfq_email`; `0014` normalizes `cage_code` (trim/upper, purge sentinels), filtered unique constraint, + `SupplierPortalChangeLog`; `0015` creates `SupplierAlias` (`contracts_supplieralias`). |
...
`Contact` holds the name/title/company/phone/email for a supplier, optionally linked to a `contracts.Address` and back to `Supplier` via `contacts`. Categories are assigned through the global `SupplierContactCategory` taxonomy via M2M (`contracts_contact_categories`). Multiple contacts per supplier may hold the **Primary** category — no uniqueness constraint. Canonical lookup: `Contact.objects.filter(supplier=supplier, categories__name="Primary")`. Assign categories via POST `suppliers:supplier_contact_set_categories` (`category_ids` comma-separated list of active category ids). First contact saved for a supplier is auto-assigned Primary. **RFQ dispatch** targets contacts with the **Sales** category (`SALES_CATEGORY_NAME` in `suppliers/contact_categories.py`); legacy `Supplier.rfq_email` is deprecated and retained only as a dormant dispatch fallback until a future column drop.

`SupplierAlias` (`contracts_supplieralias`) stores alternate/trade/DBA/legacy names for a supplier (`related_name='aliases'`). Zero-to-many per supplier. `name` is required (no `null=True`). Unique on `(supplier, name)` is a DB backstop only — the save view rejects case-insensitive duplicates before insert. Manual entry only via the supplier detail page (`contracts:supplier_save_alias` / `contracts:supplier_delete_alias`) or Django admin inline; the DIBBS scraper and portal API must never write this table. Search paths match aliases via `Q(aliases__name__icontains=…)` plus `.distinct()`.
...
2. **Supplier detail (`/suppliers/<pk>/detail/`)** – `SupplierDetailView` populates contacts, aliases (`context['aliases']` materialized with `list(...)`), documents, certifications/classifications, contract aggregates, and compliance flags. The template renders status toggles, a document list, and contract/company summaries; it also renders the `supplier_enrich.html` link/button once the user navigates to enrichment. **`name`, `supplier_type`, `prime`, and `is_packhouse` are inline-editable on the detail page via the transactions modal** (click-to-edit spans that POST through `transactions` and refresh in place). Contacts can be added, edited, and deleted inline via a modal in the Contacts section (POST to `contracts:supplier_save_contact` / `contracts:supplier_delete_contact`) without leaving the page. Aliases (AKA/DBA/legacy names) can be added and deleted inline via a modal in the Also Known As section (POST to `contracts:supplier_save_alias` / `contracts:supplier_delete_alias`); success reloads the page the same way contacts do. New addresses can be created and assigned to billing, shipping, and physical slots inline via the Add New Address modal (POST to `contracts:supplier_update_address`), including assigning one new row to multiple slots in a single save flow.
...
- `templates/suppliers/supplier_detail.html` – two-column desktop layout: a 220px sticky left sidebar (`var(--company-primary)` background) with the current supplier name, a jump search (`#sidebar-supplier-search` → `/suppliers/search/`, min 2 characters, up to 8 results), and anchor nav with count badges (contacts, aliases, contracts, certifications, documents). Main content scrolls in `.supplier-detail-main` with anchored sections: `#section-overview` (status flag banners when probation/conditional/archived are true; supplier header and health gauge side by side at 60/40 via `col-7`/`col-5`; Contact & Web and Compliance & Status side by side at 65/35 via `col-8`/`col-4` with `#section-compliance` on the right column), `#section-contacts` (contacts in a four-up Bootstrap grid — `row g-3` / `col-3`, `h-100` cards; alphabetical left-border color banding — A–F blue, G–L teal, M–R amber, S–Z purple; contacts with the **Primary** category show a red left border via `contact-card-primary`; each card is info-left with vertically stacked Edit/Delete/Categories actions on the right; assigned categories render as display-only red pills at the card bottom — **Sales** pills indicate RFQ recipients; **Categories** dropdown saves on close via POST to `suppliers:supplier_contact_set_categories`; inactive-but-assigned categories appear muted in the dropdown only), `#section-aliases` (list of AKA/DBA/legacy names with delete + Add Alias modal POSTing to `contracts:supplier_save_alias` / `contracts:supplier_delete_alias`; fetch + JSON then page reload, same pattern as contacts), `#section-addresses`, `#section-contracts`, `#section-certs` (certifications and classifications locked side by side at 50/50 via `col-6`, no wrap), `#section-files`, `#section-notes`. Notes section has an Edit button (`#notes-edit-btn`) that opens a modal (`#notesEditModal`) POSTing to `contracts:supplier_update_notes`; scroll position is preserved via sessionStorage (`supplier_notes_scroll` key). IntersectionObserver scroll spy highlights the active sidebar link (≥30% visible). Conditional status banners use `var(--bs-danger)` / `var(--bs-warning)` / `var(--bs-secondary)` as appropriate. Prime displays `prime_display` when provided, otherwise `—` for a set integer with no label (template-level; no raw ID). Edit Supplier link removed; inline transaction modal editing retained. Hidden `#enrichment-panel` preserved.
...
`admin.py` registers `Supplier` with fieldsets grouped into Basic, Contact, Branding, Addresses, Compliance & Status, and Metadata, plus a `SupplierAlias` TabularInline for AKA names. Admin staff get list filters on `archived` and `supplier_type`, searchable name/CAGE/DODAAC, and a `Contact` admin with company/email/phone search. Staff changes still rely on the `AuditModel` fields if they exist.
...
- `supplier_search_api` reads `q` and filters `name`, `aliases__name`, `cage_code`, or `clin__contract__contract_number` (the correct ORM traversal — Supplier has no direct `contract` relation, only through `Clin`) with `.distinct()` to prevent duplicate rows when multiple CLINs or aliases match. The same alias OR is applied on `SupplierSearchView` (`name` and `q` params), `supplier_autocomplete`, `SupplierListView` name/`q` filters, and the IDIQ `SupplierSearchView` JSON picker.
...
- **Supplier Health Score (added April 29, 2026):** `SupplierDetailView.get_context_data()` in `suppliers/views.py` now computes `health_score` and `health_score_alternate` dicts and passes them to `supplier_detail.html`. Logic lives in the standalone `compute_health_data(supplier, contracts_qs)` helper defined above the class. Friction signals: Notes (weight 1), GovActions (weight 3), ClinShipment corrections (weight 2, defined as `modified_on > created_on + 60s`), PaymentHistory entries (weight 1). Band thresholds: green ≥ 50 ratio, amber ≥ 15, red < 15. Data window: 24 months if ≥5 contracts in window, else all-time fallback. Both windows always computed. All weights and thresholds are hardcoded constants (`HEALTH_WEIGHTS`, `HEALTH_THRESHOLDS`) at module level in `suppliers/views.py` — TODO to make configurable per company.
...
- `/suppliers/<int:pk>/contact/<int:contact_id>/categories/` → `supplier_contact_set_categories` (POST `category_ids`).
- Additional `contracts` routes under `contracts/urls.py` expose `/suppliers/` (list), `/supplier/<pk>/` (detail/edit/create), `/supplier/<pk>/toggle-flag/`, `/supplier/<pk>/update-*` (selects, compliance, files, address), `/supplier/<pk>/contact/...`, `/supplier/<pk>/alias/save/`, `/supplier/<pk>/alias/<alias_id>/delete/`, `/supplier/<pk>/certification/...`, `/supplier/<pk>/classification/...`, `/supplier/search/`, and autocomplete/admin tooling; these call into `contracts.views.supplier_views`.
- `POST /contracts/supplier/<pk>/alias/save/` → `contracts:supplier_save_alias` — POST `name`; strips whitespace, rejects blank, case-insensitive duplicate check against existing aliases for that supplier; sets `created_by=request.user`; returns JSON `{"ok": true}` or `{"error": "..."}`.
- `POST /contracts/supplier/<pk>/alias/<alias_id>/delete/` → `contracts:supplier_delete_alias` — deletes the row scoped to that supplier; returns JSON `{"ok": true}`.
...
- Core history: `0001_initial` (creates the `contracts_*` tables and wires FKs to `contracts.Address`, `SpecialPaymentTerms`, and `User`), `0002` (adds `logo_url`, `website_url`, `primary_email`, `primary_phone`, and `last_enriched_at` to `Supplier`), `0003` (creates `suppliers.OpenRouterModelSetting` with the `needs_update` flag), `0004` (adds `Supplier.rfq_email`, now deprecated), `0005` (added `SupplierContactGroup`, removed in `0011`), `0007` (added `Contact.is_primary`, removed in `0012`), `0009`–`0013` (contact categories schema, data migration, group teardown, Sales backfill from `rfq_email`), `0014` (cage unique + portal changelog; nulls extra rows that share a CAGE before creating the filtered unique index), `0015` (`SupplierAlias` / `contracts_supplieralias`).
...
## 19. Quick Reference
- Primary models: `Supplier`, `Contact`, `SupplierAlias`, `SupplierContactCategory`, `SupplierCertification`, `SupplierClassification`, `SupplierDocument`, `OpenRouterModelSetting`.
