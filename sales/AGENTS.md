...
**The only scheduled/background worker in this app** is the Django management command `scrape_awards`, intended to run nightly via an Azure WebJob. **With no arguments, it runs a reconciliation loop:** read available dates from DIBBS (Playwright), compare to successful `AUTO_SCRAPE` batches (ORM only, after the browser closes), then scrape each missing date in separate Playwright sessions (ORM after each scrape). **`--date YYYY-MM-DD`** is for a one-off single-date scrape only. It writes into `DibbsAward` using `sales/services/dibbs_awards_scraper.py` and `awards_file_importer.import_aw_records()`.

**`scrape_awards` + mssql:** Django ORM calls cannot be made inside a `with sync_playwright()` block when running on Azure App Service with the mssql backend — the driver's `sql_server_version` property opens a temporary connection that fails inside Playwright's event loop. All DB reads must happen after the browser is closed (e.g. `get_dates_needing_scrape` only after fetching available dates); DB writes must happen after the Playwright context exits for that step.
