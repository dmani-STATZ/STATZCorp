...
- **Staging table contract:** `dibbs_award_staging` is a transient table. Rows are inserted by Python and deleted by `usp_process_award_staging` at the end of each run. The table should be empty between runs. Use `python manage.py purge_stale_award_staging --older-than-hours=24 --dry-run` before cleanup; without `--dry-run`, it deletes only rows tied to stale `IN_PROGRESS` / `FAILED` scrape batches and their matching staging-error rows. Never blanket-truncate either staging table.
- **Stored procedure:** `usp_process_award_staging` lives in `sales/sql/usp_process_award_staging.sql`. Deploy changes manually to **every environment** via SSMS using `CREATE OR ALTER PROCEDURE`; Django migrations and application deploys do not update it, so repository and live definitions can silently drift. The `ISNULL(s.pdf_url, '')` guards on both `dibbs_award` INSERT paths are mandatory because raw T-SQL bypasses Django's `default=""`. The proc also handles copying `award_basic_number_url`, `award_basic_package_view_url`, `delivery_order_number_url`, and `delivery_order_package_view_url` from staging. Startup and on-demand `python manage.py verify_stored_procs` compare that guard with the live SQL Server definition and warn on drift, but never deploy SQL. Treat `dibbs_we_won_awards` as the same manual-deploy class of database object.
...
- **`WeWonAward` SQL view contract:** `WeWonAward` is an unmanaged model backed by SQL view `dibbs_we_won_awards`. **Production (MSSQL):** create/alter the view manually in SSMS — Django migrations must not own the production DDL (`makemigrations` will not create it). **Local/CI (SQLite):** migrations `0026`, `0039`, and `0061` install a SQLite-only shim of the same view (`_drop_we_won_awards_view` / `_recreate_we_won_awards_view`, no-op when `vendor != "sqlite"`). Any future `AddField` / `AlterField` / `RemoveField` on `DibbsAward` (`dibbs_award`) must sandwich the schema ops between those helpers, or SQLite's `_remake_table` will fail with `error in view dibbs_we_won_awards: no such table`.
