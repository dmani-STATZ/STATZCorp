...
**Awards scraper + `awards_file_importer`:** The `cage_code_set` used for win detection in `_process_records()` (and the solicitation `sol_lookup` built the same way) must be pre-fetched outside any Playwright browser session and passed into `import_aw_records()` when called from the scraper path (`scrape_awards` does this before `scrape_awards_for_date()`). The default `None` behavior (fetch internally) is only safe for the file upload path (`import_aw_file` / `_process_records` without those arguments), which runs outside any browser context.

**Scraper service contract:** `scrape_awards_for_date(award_date, batch_id, on_page_complete, activity_log=None)` scrapes ONE date, calls `on_page_complete(records, page_num, total_pages)` after each page (outside browser async context), optional `activity_log` for stdout milestones (WebJob visibility), and returns a result dict. It has no loop logic, no reconciliation logic, and no DB writes — all of that is the management command's job.
