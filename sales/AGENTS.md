...
**Background processing:** Azure WebJobs run `scrape_awards`, `fetch_pending_pdfs`, and `auto_import_dibbs`. `auto_import_dibbs` Phase 4 downloads the nightly CA zip and runs `parse_ca_zip()` (procurement history + `pdf_data_pulled`); `fetch_pending_pdfs` skips sols with `pdf_data_pulled` set. The interactive solicitation import pipeline and other staff UI flows run synchronously via HTTP; the four AJAX import steps are sequential HTTP POSTs, not an internal task queue.
