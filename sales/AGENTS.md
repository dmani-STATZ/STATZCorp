...
- **Insert-path contract:** keep raw `executemany` for `DibbsAward` inserts. Do not revert to `bulk_create` due to SQL Server `OUTPUT INSERTED` behavior with `django-mssql-backend`. `DibbsAwardMod.bulk_create` is acceptable because no returned IDs are required.
