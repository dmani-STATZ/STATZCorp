...
**Scraper service contract:** `scrape_awards_for_date(award_date, batch_id, on_page_complete)` scrapes ONE date, calls `on_page_complete(records, page_num, total_pages)` after each page (outside browser async context), and returns a result dict. It has no loop logic, no reconciliation logic, and no DB writes — all of that is the management command's job.
