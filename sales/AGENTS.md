...
**Background processing:** Azure WebJobs run `scrape_awards`, `fetch_pending_pdfs`, and `auto_import_dibbs`. `auto_import_dibbs` Phase 4 fetches individual PDFs for set-aside solicitations (`small_business_set_aside != 'N'`) in one Playwright session per import date, then runs `persist_pdf_procurement_extract` outside the browser (procurement history + `pdf_data_pulled`); unrestricted (`N`) sols are skipped. `fetch_pending_pdfs` skips sols with `pdf_data_pulled` set. The interactive solicitation import pipeline and other staff UI flows run synchronously via HTTP; the four AJAX import steps are sequential HTTP POSTs, not an internal task queue.
...
- **`DibbsAward` AW import:** The scraper may re-attempt a date that was partially imported. `_process_records()` must filter out `notice_id` values that already exist before calling `executemany` to prevent `IntegrityError` on duplicate key; the same idea applies to `dibbs_award_mod` rows against the `(award, mod_date, nsn, mod_contract_price)` unique constraint. All `IN` clause queries against large record sets must be chunked using `_chunked(list, AW_CHUNK)` to stay under SQL Server's 2,100 parameter limit. This applies to `existing_keys` and `existing_mod_awards` lookups in `_process_records()`. `sales/services/awards_file_importer.py` persists rows with raw `executemany` for creates and chunked `bulk_update` for updates, not per-instance `save()`. The create path uses raw `executemany` instead of `bulk_create` because `django-mssql-backend` adds `OUTPUT INSERTED.id` to every `bulk_create` call which triggers SQL Server error 8115 regardless of data values. Do not revert to `bulk_create` for the create path. The update path uses chunked `bulk_update` which does not have this problem. Do not add `auto_now_add` / `auto_now` fields to `DibbsAward` unless every write path sets them explicitly — otherwise SQL Server can receive NULL for NOT NULL datetime columns. Import timing is modeled by `aw_file_date` and `AwardImportBatch.imported_at` (the old `synced_at` field was removed as redundant). The OUTPUT INSERTED problem affects ALL tables in this app, not just DibbsAward. DibbsAwardMod inserts also use executemany for the same reason. Any new model that uses bulk_create for inserts must be converted to executemany before deploying to SQL Server.
...
- **Insert-path contract:** keep raw `executemany` for `DibbsAward` and `DibbsAwardMod` inserts. Do not revert to `bulk_create` due to SQL Server `OUTPUT INSERTED` behavior with `django-mssql-backend`.
...
**`awards_file_importer.py`** exposes two entry points: `import_aw_file()` (file upload) and `import_aw_records()` (scraper, called once per date after the browser closes). Both delegate to shared `_process_records()` for bulk upsert, deduplication, and solicitation matching. Win detection for display and reporting uses the `WeWonAward` SQL view (`dibbs_we_won_awards`), not the `DibbsAward.we_won` column at import time.
