...
def normalize_award_record_for_importer(
    ...
    for col in COLUMNS:
        ...
        if isinstance(v, str):
            v = v.strip()
            # Strip » and everything after it (DIBBS package-view link text bleeds in)
...
def _dopostback(page, page_target: str) -> None:
    ...
    # Arm the navigation waiter *before* evaluate(); waiting after JS already ran
    # can resolve against the stale document and leave a postback in flight.
