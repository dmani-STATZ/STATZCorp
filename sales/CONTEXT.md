...
- `sales/services/dibbs_pdf.py` fetches DIBBS solicitation PDFs via Playwright (same DoD consent bypass as `dibbs_fetch.py`); `fetch_pdf_for_sol(sol_number)` returns raw ZIP/PDF bytes or `None` and runs procurement-history extraction on success; `fetch_pdfs_for_sols(sol_numbers)` returns a dict `sol_number -> bytes | None`. The RFQ queue persists blobs then calls `parse_procurement_history` / `save_procurement_history` after each successful fetch.
