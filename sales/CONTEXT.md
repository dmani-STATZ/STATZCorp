...
|---|---|
| `models/` package | Defines domain tables: `ImportBatch`, `ImportJob`, the `Solicitation` stack (including `pdf_blob`, `pdf_fetched_at`), supplier capability models (`SupplierNSN`, `SupplierFSC`, `ApprovedSource`), RFQ/quote/bid records (`SupplierRFQ` with `QUEUED` status), `RFQGreeting`, `RFQSalutation`, `NoQuoteCAGE`, `CompanyCAGE`, `EmailTemplate`, `DibbsAward`, and match/contact-log data. Many tables reuse `suppliers.Supplier`. |
...
| `services/dibbs_pdf.py` | Fetches DIBBS solicitation PDFs via Playwright (same DoD consent bypass as `dibbs_fetch.py`); `fetch_pdf_for_sol(sol_number)` and `fetch_pdfs_for_sols(sol_numbers)` are used by the RFQ queue “Fetch PDFs for Selected” action. |
...
| `services/awards_file_importer.py` | Bulk-upserts `DibbsAward` from parse result; creates `AwardImportBatch`; matches solicitations; `we_won` is set to True when `awardee_cage` matches any active `CompanyCAGE.cage_code` (case-insensitive); uses synthetic `notice_id` (`DF` + SHA-256) for file rows. |
| `views/awards.py` | Staff-only upload view, import result view (session key `aw_import_result`), filterable awards list. |
...
| `migrations/` | Tracks schema evolution from 0001 through 0019+ (includes `NoQuoteCAGE` / `dibbs_no_quote_cage`, buckets, RFQ extras, bids, import jobs, awards + AW import batch, email templates, manual match method, CAGE/IMAP changes, solicitation PDF fields, RFQ greeting/salutation tables). The `suppliers` app has a separate migration for `Supplier.rfq_email`. |
...
- **Solicitation stack:** `Solicitation` holds numbers, status, buckets, return dates, optional `pdf_blob` (BinaryField) and `pdf_fetched_at` (DateTimeField) for stored DIBBS PDFs, and links to `SolicitationLine`. `SolicitationLine` has NSN text, quantities, delivery days, and `bq_raw_columns` JSON (copy of the BQ row) for later export. Each line connects to matches, quotes, and bids.
...
- **Awards:** `DibbsAward` stores both SAM.gov entries (source=`'SAM'`) and DIBBS AW file entries (source=`'DIBBS_FILE'`). New fields: `award_basic_number`, `delivery_order_number`, `awardee_cage`, `total_contract_price`, `award_date`, `nsn`, `nomenclature`, `dibbs_solicitation_number`, `aw_import_batch`. Ties to `Solicitation` when possible. Flags `we_bid`/`we_won`. Persists `sam_data` for SAM diagnostics. `AwardImportBatch` (table `dibbs_award_import_batch`) tracks each AW file upload with row/created/updated/we_won counts.
...
- `sales/base.html` supplies the main navigation (Dashboard, Solicitations, RFQ Center, Bid Center, Suppliers, Import, Awards, Bid History, Settings, No Quote CAGEs, Email templates) plus a top-bar search that targets `sales:global_search`. Most screens are server-rendered with minimal JavaScript.
- Import templates: `sales/import/upload.html`, the multi-step `sales/import/progress.html` with AJAX polling/checklists, and `sales/import/history.html`. Awards: `sales/awards/import_upload.html`, `sales/awards/import_result.html`, `sales/awards/list.html`.
- Solicitations use `sales/solicitations/list.html` (four tabs — Matches / Set-Asides / Unrestricted / No-Bid; pipeline tabs exclude `NO_BID`; filter bar; column sort; bulk reassignment; row links append `list_qs` from `filter_snapshot`) and `sales/solicitations/detail.html` (optional prev/next + back-to-list nav when `list_qs` is set; pipeline ribbon; status banners for active pipeline statuses; **Matches** tab: matched suppliers and approved sources wired to the RFQ queue; SAM modal for create-supplier-and-queue; RFQ/quote/bid tabs; contact log; bid builder context).
- RFQ screens include `sales/rfq/pending.html`, `sales/rfq/center.html`, `sales/rfq/queue.html` (full-width queue by supplier), plus `sales/rfq/partials/center_panel.html` and `sales/rfq/partials/mailto_buttons.html`, `sales/rfq/sent.html`, and `sales/rfq/quote_entry.html`.
...
- Staff-only endpoints (`backfill_nsn`, `sync_awards_view`, `awards_import_upload`) check `request.user.is_staff` or use `@user_passes_test`.
...
- `sales/services/dibbs_pdf.py` fetches DIBBS solicitation PDFs via Playwright (same DoD consent bypass as `dibbs_fetch.py`); `fetch_pdf_for_sol(sol_number)` returns raw PDF bytes or `None`; `fetch_pdfs_for_sols(sol_numbers)` returns a dict `sol_number -> bytes | None`. Not yet wired to the queue.
...
- `/awards/` (list, filterable), `/awards/import/` (staff upload), `/awards/import/result/` (import summary), `/awards/sync/` (staff-only manual SAM sync)
...
- `rfq_inbox_list` treated IMAP as configured only when `cage.imap_password` was set, but `imap_password` was removed from `CompanyCAGE` (migration `0015`; IMAP is intended to use delegated OAuth in `imap_fetch`). A `getattr` guard on `imap_host`, `imap_user`, and `imap_password` avoids `AttributeError` so the inbox page loads with `imap_configured=False` until IMAP readiness is defined. Full IMAP integration is deferred where not built. The tenant is GCC High — a non-production test mailbox is not available, so IMAP must be tested against production when built.
...
## 19. Quick Reference
- **Primary models:** `ImportBatch`, `Solicitation`/`SolicitationLine`, `SupplierMatch`, `SupplierRFQ`, `SupplierQuote`, `GovernmentBid`, `CompanyCAGE`, `EmailTemplate`, `RFQGreeting`, `RFQSalutation`, `NoQuoteCAGE`, `DibbsAward`, `AwardImportBatch`, `SupplierNSN`/`SupplierFSC`, `ApprovedSource`.
- **Main URLs:** `/sales/import/*`, `/sales/awards/*` (list, import, result) and `/sales/awards/sync/`, `/sales/solicitations/*`, `/sales/rfq/*` (including `supplier_create_and_queue`), `/sales/bids/*`, `/sales/suppliers/*`, `/sales/settings/*` (cages, email, greetings, salutations, **no-quote**), `/sales/settings/no-quote/`, `/sales/entity/cage/<cage_code>/` (`?fmt=json` supported).
- **Key templates:** `sales/import/progress.html`, `sales/solicitations/list.html`, `sales/solicitations/detail.html`, `sales/rfq/center.html` plus `rfq/partials/mailto_buttons.html`, `sales/bids/builder.html`, `sales/settings/email_templates.html`, `sales/settings/greetings.html`, `sales/settings/salutations.html`.
