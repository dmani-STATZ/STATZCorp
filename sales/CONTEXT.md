...
| `services/dibbs_fetch.py` | Scrapes DLA’s RFQDates page with `requests`/`BeautifulSoup`, automates Playwright consent/download flows, and extracts IN/BQ/AS files into a temp directory for import. Also scrapes optional CA zip hrefs per date and provides `fetch_ca_zip(url)` for bulk SF-18 zips. |
...
| `services/ca_parser.py` | `parse_ca_zip(zip_bytes, import_date)` — processes DIBBS CA zip in memory, looks up `Solicitation` by `pdf_file_name`, skips sols with `pdf_data_pulled` set, parses procurement history, saves rows, updates `pdf_data_pulled`. Returns result summary dict. |
...
- `sales/services/dibbs_fetch.py` performs DIBBS discovery with `requests`/`BeautifulSoup` and drives Playwright automation to download IN/BQ/AS files for import; optional per-date CA zip links are scraped and `fetch_ca_zip()` downloads bulk SF-18 archives for `parse_ca_zip()`.
...
**`auto_import_dibbs`** — Daily management command (Azure WebJob, 2:00 AM CT, `0 0 7 * * *`). Phase 1: scrapes RFQDates.aspx for available dates. Phase 2: reconciles against `ImportBatch` — queues any DIBBS date that has both IN and BQ links but no `ImportBatch` row yet (`ImportBatch` has no error/status fields; a row means that date was imported). Phase 3: fetches files via `fetch_dibbs_archive_files()` (Playwright) then calls `run_import()` (ORM) for each date, oldest first. Phase 4: after IN/BQ/AS import succeeds for a date, downloads `ca{yymmdd}.zip` via `fetch_ca_zip()` (Playwright), passes bytes to `parse_ca_zip()` (ORM) which extracts each PDF in memory, parses procurement history via `parse_procurement_history()`, saves via `save_procurement_history()`, and sets `pdf_data_pulled` on matched `Solicitation` rows. Parse-and-discard — no `pdf_blob` stored. If the CA href is missing for a date, that step is skipped without error. Consolidated failure alert: sends to `AWARDS_ALERT_EMAIL` via Graph mail if any date failed (including CA zip fetch/parse failures). ORM/Playwright boundary rule applies. Manual import flow unchanged.

**`fetch_pending_pdfs`** — Queries `Solicitation` for `pdf_fetch_status` in `PENDING` or `FAILED` (retry) with `pdf_fetch_attempts < 5` and `pdf_data_pulled` null (skips sols already covered by the CA zip pipeline). Opens one shared Playwright browser session via `fetch_pdfs_for_sols()`, fetches all matching PDFs, saves blobs, runs procurement history parse/save. Marks `DONE` on success, `FAILED` on failure (increments `pdf_fetch_attempts`). After five failures a sol is permanently skipped. Designed to run every five minutes as an Azure WebJob during office hours. All ORM calls happen outside the Playwright session boundary (Azure mssql requirement).
...
- `0031_solicitation_pdf_data_pulled`: adds `pdf_data_pulled` to `Solicitation` (procurement history extracted via CA zip pipeline; `fetch_pending_pdfs` skips when set).
