...
|---|---|
| `models/` package | Defines domain tables: `ImportBatch`, `ImportJob`, the `Solicitation` stack (including `pdf_blob`, `pdf_fetched_at`), supplier capability models (`SupplierNSN`, `SupplierFSC`, `ApprovedSource`), RFQ/quote/bid records (`SupplierRFQ` with `QUEUED` status), `RFQGreeting`, `RFQSalutation`, `CompanyCAGE`, `EmailTemplate`, `DibbsAward`, and match/contact-log data. Many tables reuse `suppliers.Supplier`. |
...
| `migrations/` | Tracks schema evolution from 0001 through 0017 (buckets, RFQ extras, bids, import jobs, awards, email templates, manual match method, CAGE/IMAP changes, solicitation PDF fields, RFQ greeting/salutation tables). The `suppliers` app has a separate migration for `Supplier.rfq_email`. |
...
2. **Solicitation browsing:** `/sales/solicitations/` offers three filter tabs (Matches, Set-Asides, Unrestricted) plus a filter bar (set-aside, status, line item type, full-text) and bulk bucket reassignment. Tabs are query filters: Matches = at least one `SupplierMatch`; Set-Asides = `small_business_set_aside` not unrestricted; Unrestricted = set-aside unrestricted. A solicitation can appear in more than one tab. Detail view (`solicitations/<sol_number>/`) shows the pipeline ribbon, **Matches** tab with queue-only actions (matched suppliers and in-system approved sources use **+ Add to Queue** / `rfq_queue_add`; not-in-system approved sources use **+ Add & Queue**, which opens a SAM.gov modal), RFQ/quote/bid tabs, approved sources, contact log, and embedded bid builder context. Context includes `queued_supplier_ids` for per-supplier queued state. Status transitions: adding to queue advances `New`/`Matching` → `RFQ_PENDING`; sending from queue advances `RFQ_PENDING` → `RFQ_SENT`. `/sales/search/` supplies top-bar typeahead and JSON results.
...
6. **Settings & SAM:** `/sales/settings/` redirects to the `CompanyCAGE` list; add/edit forms adjust compliance codes, markup, and SMTP reply-to, ensuring only one default cage. The settings landing links to **RFQ Greetings** and **RFQ Salutations** (list + inline add, toggle active, delete). `/sales/settings/email/` lists templates, `email_template_edit` manages creation/update, and `email_template_preview` renders sample data via `_SafeDict`. `/sales/entity/cage/<cage_code>/` calls `sam_entity.lookup_cage`, handles missing API keys/errors gracefully, and renders SAM metadata (staff sees raw JSON); `?fmt=json` returns the same lookup as structured JSON for client-side prefill (e.g. solicitation detail SAM modal). The suppliers app supplier profile page includes an RFQ Email widget (picker for business/primary/contact emails or manual entry) that POSTs to `suppliers:supplier_set_rfq_email` to set `Supplier.rfq_email`.
...
- Solicitations use `sales/solicitations/list.html` (three filter tabs — Matches / Set-Asides / Unrestricted — filter bar, bulk reassignment) and `sales/solicitations/detail.html` (pipeline ribbon; **Matches** tab: matched suppliers and approved sources wired to the RFQ queue; SAM modal for create-supplier-and-queue; RFQ/quote/bid tabs; contact log; bid builder context).
...
- Settings screens: `sales/settings/cages.html`, `sales/settings/cage_form.html`, `sales/settings/email_templates.html`, `sales/settings/email_template_form.html`, `sales/settings/greetings.html`, `sales/settings/salutations.html`, with the email preview endpoint providing JSON for live previews.
...
- `/settings/` cages, cage add/edit, email template list/edit/new/delete/set-default, preview, greetings and salutations (list/add/delete/toggle)
- `/awards/sync/` (staff-only manual sync)
- `/entity/cage/<cage_code>/` (optional `?fmt=json` for structured SAM data without HTML)
...
## 19. Quick Reference
- **Primary models:** `ImportBatch`, `Solicitation`/`SolicitationLine`, `SupplierMatch`, `SupplierRFQ`, `SupplierQuote`, `GovernmentBid`, `CompanyCAGE`, `EmailTemplate`, `RFQGreeting`, `RFQSalutation`, `DibbsAward`, `SupplierNSN`/`SupplierFSC`, `ApprovedSource`.
- **Main URLs:** `/sales/import/*`, `/sales/solicitations/*`, `/sales/rfq/*` (including `supplier_create_and_queue`), `/sales/bids/*`, `/sales/suppliers/*`, `/sales/settings/*` (cages, email, greetings, salutations), `/sales/entity/cage/<cage_code>/` (`?fmt=json` supported).
- **Key templates:** `sales/import/progress.html`, `sales/solicitations/detail.html`, `sales/rfq/center.html` plus `rfq/partials/mailto_buttons.html`, `sales/bids/builder.html`, `sales/settings/email_templates.html`, `sales/settings/greetings.html`, `sales/settings/salutations.html`.
