...
- Check `sales/context_processors.py` — adds `overdue_rfq_count` to every request; **`solicitation_nav_tools`** adds `show_cost_of_money_calculator` and `cost_of_money_rate` when the path is under `/sales/solicitations/` (Cost of Money control in `sales/base.html`).
