...
SELECT        line.solicitation_id, SUM(ISNULL(t1.cnt, 0) + ISNULL(t2.cnt, 0) + ISNULL(t3.cnt, 0)) AS match_count
FROM            dbo.dibbs_solicitation_line AS line LEFT OUTER JOIN
                             (SELECT        nsn, COUNT(*) AS cnt
                               FROM            dbo.dibbs_supplier_nsn_scored
                               GROUP BY nsn) AS t1 ON t1.nsn = REPLACE(line.nsn, '-', '') LEFT OUTER JOIN
                             (SELECT        a.nsn, COUNT(*) AS cnt
                               FROM            dbo.tbl_ApprovedSource AS a INNER JOIN
                                                         dbo.contracts_supplier AS s ON s.cage_code = a.approved_cage AND s.archived = 0
                               GROUP BY a.nsn) AS t2 ON t2.nsn = REPLACE(line.nsn, '-', '') LEFT OUTER JOIN
                             (SELECT        fsc_code, COUNT(*) AS cnt
                               FROM            dbo.dibbs_supplier_fsc
                               GROUP BY fsc_code) AS t3 ON t3.fsc_code = LEFT(REPLACE(line.nsn, '-', ''), 4)
