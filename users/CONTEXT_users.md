...
- **The list schema is discovered at runtime** via `_get_writable_list_columns()` (cached per list id). `_build_sp_fields()` only sends columns that actually exist and are writable, so an IT-managed schema change degrades to dropped fields instead of a 400 that kills the whole push. Title/start/end column names are resolved from the same candidate tuples the read side tolerates (`_TITLE_COLUMN_CANDIDATES`, `_START_COLUMN_CANDIDATES`, `_END_COLUMN_CANDIDATES`). A list with no writable start/end column raises rather than pushing dateless items.
- `is_private=True` events are never pushed — the SharePoint list is company-wide. Flipping an existing event to private deletes its SharePoint item and clears `sharepoint_id`.
- Settings: `SHAREPOINT_CALENDAR_PUSH_ENABLED` (default `True`) is the master switch. `SHAREPOINT_CALENDAR_BACKFILL_ENABLED` (default `False`) gates the backlog sweep, bounded by `SHAREPOINT_CALENDAR_BACKFILL_SINCE_DAYS` (default 30) measured against `start_at` — enabling it writes existing portal events onto the shared calendar, so it is a deliberate one-time decision.
- **Known gap — recurring events push as a single item.** A `WorkCalendarEvent` with a `RecurrenceRule` sends only its base occurrence to SharePoint; the series is not expanded and no SharePoint recurrence is set. This matches the pull side, which does not read recurrence either. Real support needs the list's `fRecurrence` / `RecurrenceData` XML columns, which are schema-dependent — do not fake it by pushing one item per expanded occurrence, that would flood the list and break the `sharepoint_id` one-to-one link.
- **Known gap — failed *edits* are not retried.** The backlog sweep only picks up events with no `sharepoint_id`, so a failed *create* is retried but a failed *update* leaves stale data in SharePoint until the event is edited again. This is deliberate: the obvious retry condition (`updated_at > sharepoint_last_modified`) is true for every row the pull just wrote, which would ping-pong events between the two systems on every WebJob tick. Any future fix needs a real dirty-flag on the model, not a timestamp comparison.
...
- `sharepoint_services._uncorrect_sharepoint_datetime()` is the exact inverse and is applied to every start/end **leaving** Django for Graph: stored value → Central wall-clock → strip tz → re-localize as Pacific → UTC ISO `Z` string. The two functions are a matched pair; an event pushed and then pulled back lands on the identical instant. Never send raw UTC on the push side.
- Env var `SHAREPOINT_SOURCE_TIMEZONE` (default `America/Los_Angeles`) drives the source-timezone step in both directions. If IT changes the SharePoint site regional setting, update this env var (for example to `America/Chicago`) without a code deploy.
...
- **Import:** `users/management/commands/import_release_notes.py` scans `release_notes/*.md` (under `settings.BASE_DIR`), validates YAML frontmatter and tag taxonomy (`users/release_notes/constants.py`), and upserts `ReleaseNote` rows. Fail-soft: warnings and summary counts; exit code 0 always. `published` is a file-only gate (not a model field): `false` skips the file and does not update or hide an existing DB row — un-publishing after a prior import requires manual row cleanup.
...
- Azure WebJobs (or similar) may run `python manage.py run_background_tasks` from the `core` app; that command invokes registered tasks including `users.tasks.sync_calendar.run()` (two-way SharePoint list ↔ WorkCalendarEvent sync via sharepoint_services.sync_sharepoint_calendar_two_way).
