...
14. **SharePoint calendar timezone correction:** `sharepoint_services._correct_sharepoint_datetime()` is required for every Graph start/end datetime entering `WorkCalendarEvent` from `sync_sharepoint_calendar()`. Its inverse, `_uncorrect_sharepoint_datetime()`, is required for every start/end leaving Django for Graph in `_build_sp_fields()`. Do not bypass either or parse/emit Graph UTC values directly for those fields — the SharePoint site is Pacific-configured while users type Central, so raw Graph UTC is offset-corrupted in both directions. They are a matched pair: changing one without the other breaks the push/pull round trip. Behavior is driven by env var `SHAREPOINT_SOURCE_TIMEZONE`.
...
16. **Calendar push is one module, not two.** All SharePoint calendar traffic — both directions — lives in `users/sharepoint_services.py` and uses the `GRAPH_MAIL_*` service principal with `SHAREPOINT_CALENDAR_SITE_ID` / `SHAREPOINT_CALENDAR_LIST_ID`. A second implementation (`users/sharepoint_sync.py`, class `SharePointCalendarSync`) previously existed that resolved the site by URL and the list by display name using the **user-OAuth** app registration; it was never able to write and was removed 2026-08-17. Do not reintroduce a parallel sync module or a second credential path.

17. **Never send SharePoint column names you have not verified exist.** `_build_sp_fields()` filters every field through `_get_writable_list_columns()`. The calendar list is IT-managed and its schema is not guaranteed. Graph rejects the entire request when one unknown column is present, so a hard-coded field name takes down the whole push, not just that field.

18. **Pushes must never break a user's save.** `portal_event_create/update/delete` call `schedule_event_push()` / `schedule_event_delete()`, which wrap the Graph call in `transaction.on_commit` and swallow exceptions. Do not call `push_event_to_sharepoint()` directly from a request path, and do not let a Graph error propagate into a view — a failed push is recoverable via the backlog sweep, a failed save is not.

19. **Private events never leave Django.** The SharePoint calendar list is company-wide. `event_is_pushable()` blocks `is_private=True`, and `portal_event_update` withdraws the SharePoint item when an event is flipped to private. Any new push path must honor the same check.

20. **Do not reassign `organizer` for portal-origin events during a pull.** `sync_sharepoint_calendar()` only rewrites `organizer` / `source_identifier` when `existing.source_system == "sharepoint"`. A portal event that we pushed up is mirrored in SharePoint, not owned by it — reassigning its organizer to `SHAREPOINT_SYNC_USER_EMAIL` strips the creator's edit and delete rights, since `portal_event_update` / `portal_event_delete` both gate on `organizer_id`.
